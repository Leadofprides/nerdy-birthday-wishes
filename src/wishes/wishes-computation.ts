...
const computeHashWithNonce = (wishes_body: WishesCreateDto, pow_nonce: number): string => {
...
while (true) {
    ...
    if (hash.startsWith('00')) {
        ...
        parentPort.postMessage({ pow_nonce, hash, computation_started_at, computation_finished_at, thread_id: threadId });
        break;
    }
    pow_nonce++;
}
