...
export class WishesService {
    ...
    async prepareWishes(wishes_body: WishesCreateDto): Promise<WishesUuidResponseDto> {
    ...
    private startWorker(wishes_uuid: string, wishes_body: WishesCreateDto) {
        ...
        worker.on('message', async (data) => {
            const wish = await this.wishesRepository.findOne({ where: { wishes_uuid } });
    ...
    async checkWishesStatus(wishes_uuid: string): Promise<WishesStatusDto> {
        ...
        if (!wish) {
            throw new NotFoundException(`Wish with wishes_uuid ${wishes_uuid} not found`);
